...
export function HomeGreynPreview() {
  ...
  useEffect(() => {
    ...
    if (reducedMotion) {
      gsap.set([contentRef.current, lineRef.current, headingRef.current, statementRef.current, ...itemEls, ctaRef.current].filter(Boolean), { opacity: 1, y: 0, x: 0, scaleX: 1 });
      return () => {};
    }
    gsap.set(contentRef.current, { opacity: 0, x: -24, scale: 0.98 });
    gsap.set(lineRef.current, { scaleX: 0, transformOrigin: "left center" });
    gsap.set(headingRef.current, { opacity: 0, y: 20 });
    gsap.set(statementRef.current, { opacity: 0, y: 16 });
    ...
    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        ...
        onEnter: () => {
          ...
          gsap.to(statementRef.current, { opacity: 1, y: 0, duration: 0.55, delay: 0.2, ease: "power3.out" });
        ...
        onEnterBack: () => {
          gsap.to([contentRef.current, lineRef.current, headingRef.current, statementRef.current, ...itemEls, ctaRef.current].filter(Boolean), {
            opacity: 1,
            y: 0,
            x: 0,
            scale: 1,
  ...
  return (
    <section
      ...
      aria-labelledby="greyn-preview-heading"
    >
      <div ref={contentRef} className="max-w-2xl mx-auto">
        <div ref={lineRef} className="h-0.5 w-12 mb-6 bg-[var(--color-accent-cyan)]" style={{ transformOrigin: "left center" }} aria-hidden />
        <h2
          id="greyn-preview-heading"
          ref={headingRef}
        ...
        <ul className="space-y-3 mb-10">
          {HIGHLIGHTS.map((text, i) => (
            ...
            >
              <span className="w-1.5 h-1.5 rounded-full bg-[var(--color-accent-cyan)] shrink-0" aria-hidden />
              {text}
            </li>
          ))}
        </ul>
